...
public class BurpReqRespTools {
     ...
     */
    public static IHttpRequestResponse makeBurpReqRespFormOkhttp(Call call, Response response, IHttpService httpService){
        IHttpRequestResponse messageInfo_r = new HttpRequestResponseFactory();;//根据响应构造burp的IHttpRequestResponse对象
        byte[] ok_reqInfo = okhttpReqToburpReq(call, response);
        byte[] ok_respInfo = okhttpRespToburpResp(call, response);
//        CommonStore.callbacks.printOutput("####################Request and Response###########################\n" +
//                "VulTaskImpl-reqInfo \n" + new String(ok_reqInfo) +
//                "\n-----------------------------------------------\n"+
//                "VulTaskImpl-respInfo \n" + new String(ok_respInfo)
//        );
        messageInfo_r.setRequest(ok_reqInfo);
        messageInfo_r.setResponse(ok_respInfo);
        messageInfo_r.setHttpService(httpService);
        return  messageInfo_r;
    }
     ...
     * @return byte[]
     */
    public static byte[] okhttpReqToburpReq(Call call, Response response){
        ...
        String ok_protocol = response.protocol().toString().toUpperCase();//HTTP/1.1必须要大写
        ...
        try {//为空会报错，但是get请求体就是为空的
            ...
            //保持默认值空字符串即可
            CommonStore.callbacks.printError("[okhttpReqToburpReq] ok_reqBody charset Error -> " + e.getMessage());
        }
     ...
     * @return byte[]
     */
    public static byte[] okhttpRespToburpResp(Call call, Response response){
    ...
    public static IRequestInfo getRequestInfo(IHttpRequestResponse requestResponse){
        return CommonStore.helpers.analyzeRequest(requestResponse);
    }
    ...
    public static IResponseInfo getRespuestInfo(IHttpRequestResponse requestResponse) {
        return CommonStore.helpers.analyzeResponse(requestResponse.getResponse());
    }
    ...
    public static short getStatus(IHttpRequestResponse requestResponse) {
        if (requestResponse != null) {
            return CommonStore.helpers.analyzeResponse(requestResponse.getResponse()).getStatusCode();
        }
        return -1;
    }
}
